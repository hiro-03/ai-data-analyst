...
注:
- モバイル（Flutter）は **Amplify Auth による USER_SRP_AUTH（SRP）** と整合させる。
- 本スクリプトは **CI 専用** であり、OIDC ロールの IAM 権限で **ADMIN_USER_PASSWORD_AUTH**
  を使う（SRP の多段チャレンジを boto3 で再実装する必要がない）。
- ADMIN フローは template.yaml の IsNotProd により **ステージングのユーザープールクライアントのみ**
  で有効。本番クライアントには含めない。
